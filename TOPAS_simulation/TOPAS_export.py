...
#Inside the input folder of topas, a folder for each patient will be created, and inside this folder the folders for each beam, containing the files for each control point

#Note that you need to manually change the includeFile='inputs.txt' inside beam.txt to the name of the file that corresponds to each beam, for example: Protons_01/Beam_1/inputs.txt
#And inside plan_hu.txt, includeFile='plan.txt' to : Protons_01/plan.txt
...
parser.add_argument("-o", "--OutputTOPASPath", nargs='?', help="The output directory where the files will be saved. It should be the one containing the beam.txt and plan_hu.txt files.", default="/rti.git/rti/topas/Protons")
parser.add_argument("-i", "--inputDICOMPath", nargs='?', help="The input directory containing the DICOM files of the different proton patients generated by writeTROTStoDICOM.py, format: C:\ Your\Directory\DICOMs\Protons", default="/tmp")
parser.add_argument("-d", "--ParticlesperHistory",type=float, nargs='?', help="Decimation factor of particles in RTplan, or by how much the number of histories is scaled down to have a reasonable computation time", default=1)

args = parser.parse_args()

#Since Topas has trouble reading Windows directories, we will change the format of the directories to Linux/WSL ones
...
print(f"Input DICOM path: {args.inputDICOMPath}")
print(f"Output TOPAS path: {args.OutputTOPASPath}")
print("-----------------------------------\n")

for patient_folder in os.listdir(args.inputDICOMPath):
    patient_path = os.path.join(args.inputDICOMPath, patient_folder)
    if os.path.isdir(patient_path):
        print(f"Processing patient folder: {patient_folder}")
        #We will create a folder for each patient
        patient_folder_path = os.path.join(args.OutputTOPASPath, f"{patient_folder}")
